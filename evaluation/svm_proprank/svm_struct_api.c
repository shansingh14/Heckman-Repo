...
double rankofpos(LABEL y, LABEL ybar);
...
SAMPLE      read_struct_examples(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  for(k=0;k<sample.n;k++) {
    ...
    for(i=0;i<y->totdoc;i++) {
      ...
      if(y->class[i] > 0) {
	if(x->scaling != 0) {
	  printf("Multiple relevant documents were found in qid %ld.\nERROR: Only one relevant doc per qid allowed.\n",k);
	  exit(1);
	}
	x->scaling=x->doc[i]->costfactor;  /* for Propensity weighting */
	y->ipw=x->doc[i]->costfactor; 
      }
    }
    totpairs=0;
    for(i=0;i<y->totdoc;i++) {
    ...
    if(x->scaling <= 0) {
      printf("ERROR: Missing propensity weight in qid %ld\n",k);
      exit(1);
    }
  ...
  if(sparm->loss_function == SWAPPEDPAIRS) {
    ...
    for(k=0;k<sample.n;k++) {
      x=&sample.examples[k].x;
      /*      x->scaling=1.0; Removed for propensity weighting */
    }
  }
				       ...
				       STRUCT_TEST_STATS *teststats)
{
  ...
  printf("Avg Rank of Positive Examples: %6.2f (via SNIPS Estimator [Swaminathan & Joachims, 2015d])\n",teststats->rankofpos/teststats->numpos);
}
			    ...
			    STRUCT_TEST_STATS *teststats)
{
  ...
  if(exnum == 0) { /* this is the first time the function is
    ...
    teststats->rankofpos=0;
    teststats->numpos=0;
    teststats->fracswappedpairs=0;
  }
  teststats->swappedpairs+=swappedpairs(ex.y,ypred);
  teststats->rankofpos+=ex.y.ipw*rankofpos(ex.y,ypred); /* use propensity */
  teststats->numpos+=ex.y.ipw; 
...
void        print_struct_help()
{
  ...
  printf("    %2d  IPS-weighted rank of relevant documents.\n\n",SWAPPEDPAIRS);
  printf("NOTE: When the propensities of all relevant train docs are equal to 1, then\n");
  printf("      SVM-light in '-z p' mode (with c_light = c_rank/n, where n is the number\n");
  printf("      of training clicks) and unweighted SVM-rank with Swapped Pairs loss are\n");
  printf("      equivalent.\n\n");
  printf("The algorithms implemented in Propensity SVM-rank are described in:\n");
  printf("- T. Joachims, A. Swaminathan, T. Schnabel. Unbiased Learning-to-Rank with\n");
  printf("  Biased Feedback. Proceedings of the International Conference on Web Search\n");
  printf("  and Data Mining (WSDM), 2017.\n");
...
double rankofpos(LABEL y, LABEL ybar)
{
  /* Returns the sum of ranks of the relevant documents for                     
  ...
  for(i=0;i<y.totdoc;i++) {
    for(j=0;j<y.totdoc;j++) {
      if((y.class[i]==1) && (i != j) && (ybar.class[i]<=ybar.class[j])) {
        sum++;
      }
    }
  }
